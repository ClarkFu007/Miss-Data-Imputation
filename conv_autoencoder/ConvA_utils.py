...
def merge_feature(data_set, num_cols):
    """
       Merge features for the dataset.
    :param data_set: dataset to merge fentures
    :param num_cols: the columns for numerical features

    :return: dateset after emerging features
    """
    x_data = data_set[:, :, :, 0:len(num_cols)]
    # After merging features
    x_data_fe = np.zeros((data_set.shape[0], data_set.shape[1],
                          data_set.shape[2] * len(num_cols)), dtype='float')
    for sample_i in range(data_set.shape[0]):
        for time_i in range(data_set.shape[2]):
            index_j = len(num_cols) * time_i
            index_k = len(num_cols) * time_i + len(num_cols)
            x_data_fe[sample_i, :, index_j:index_k] = copy.deepcopy(x_data[sample_i, :, time_i])
    return x_data_fe
...
def preprocess_data(x_set, cx_set):  # Pass by reference!
    ...
    for sample_i in range(x_set0.shape[0]):
        scaler_x = preprocessing.MinMaxScaler()
        scaler_x.fit(x_set0[sample_i])
        x_set0[sample_i] = scaler_x.transform(x_set0[sample_i])

        scaler_cx = preprocessing.MinMaxScaler()
        scaler_cx.fit(cx_set0[sample_i])
        cx_set0[sample_i] = scaler_cx.transform(cx_set0[sample_i])
